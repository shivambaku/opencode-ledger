...
export function DiffLine(props: { line: string; width: number; scrollX: number; kind: VisibleDiffKind; active: boolean; blockActive: boolean; explanationActive: boolean; blockResolved: boolean; path?: string; filetype?: string; syntaxStyle: SyntaxStyle }) {
  ...
  return (
    <box width={width()} overflow="hidden" flexDirection="row" backgroundColor={backgroundColor()}>
      ...
      <Show when={codeLine()} fallback={<text width={contentWidth()} fg={textColor()} wrapMode="none">{visibleContent()}</text>}>
        <code width={contentWidth()} content={visibleContent()} filetype={props.filetype} syntaxStyle={props.syntaxStyle} drawUnstyledText={true} wrapMode="none" />
      </Show>
    </box>
  )
}
